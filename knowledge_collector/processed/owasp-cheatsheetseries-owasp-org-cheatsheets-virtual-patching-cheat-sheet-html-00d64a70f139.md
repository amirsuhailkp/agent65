...
- security
- vulnerability
date_collected: '2026-07-26T12:37:05.701263Z'
language: unknown
---
...
## Definition: Virtual Patching[¶](#definition-virtual-patching)
...
## Why Not Just Fix the Code[¶](#why-not-just-fix-the-code)
...
## Value of Virtual Patching[¶](#value-of-virtual-patching)
...
## Virtual Patching Tools[¶](#virtual-patching-tools)
...
## A Virtual Patching Methodology[¶](#a-virtual-patching-methodology)
...
## Example Public Vulnerability[¶](#example-public-vulnerability)
...
script not properly sanitizing user-supplied input to the
```
reqID
```
...
- **Increase HTTP Audit Logging**– The standard Common Log Format (CLF) utilized by most web servers does not provide adequate data for conducting proper incident response. You need to have access to the following HTTP data:
  - Request URI (including QUERY\_STRING)
...
The Identification Phase occurs when an organization becomes aware of a vulnerability within their web application. There are generally two different methods of identifying vulnerabilities:
```
Proactive
```
...
### Proactive Identification[¶](#proactive-identification)
...
### Reactive Identification[¶](#reactive-identification)
...
## Virtual Patch Creation Phase[¶](#virtual-patch-creation-phase)
...
### Manual Virtual Patch Creation[¶](#manual-virtual-patch-creation)

#### Positive Security (Allow List) Virtual Patches (**Recommended Solution**)[¶](#positive-security-allow-list-virtual-patches-recommended-solution)
...
This virtual patch will inspect the
```
reqID
```

parameter value on the specified page and prevent any characters other than integers as input.
...
#### Negative Security (Block List) Virtual Patches[¶](#negative-security-block-list-virtual-patches)
...
##### Example Block List ModSecurity Virtual Patch[¶](#example-block-list-modsecurity-virtual-patch)
...
#### Which Method is Better for Virtual Patching – Positive or Negative Security[¶](#which-method-is-better-for-virtual-patching-positive-or-negative-security)
...
#### Beware of Exploit-Specific Virtual Patches[¶](#beware-of-exploit-specific-virtual-patches)
...
### Automated Virtual Patch Creation[¶](#automated-virtual-patch-creation)
...
## Implementation/Testing Phase[¶](#implementationtesting-phase)
...
## Recovery/Follow-Up Phase[¶](#recoveryfollow-up-phase)
