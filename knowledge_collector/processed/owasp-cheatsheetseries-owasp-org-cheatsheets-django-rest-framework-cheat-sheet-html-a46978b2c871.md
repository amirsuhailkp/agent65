...
- classes
- framework
date_collected: '2026-07-26T12:36:22.871506Z'
language: unknown
---

# Django REST Framework (DRF) Cheat Sheet[¶](#django-rest-framework-drf-cheat-sheet)
...
## What is a view in Django?[¶](#what-is-a-view-in-django)
...
### DEFAULT\_THROTTLE\_CLASSES[¶](#default_throttle_classes)
...
### DEFAULT\_PAGINATION\_CLASS[¶](#default_pagination_class)
...
## OWASP API Security Top 10 (2019)[¶](#owasp-api-security-top-10-2019)
...
without checking if the request should have access to that object.

### API2:2019 Broken User Authentication[¶](#api22019-broken-user-authentication)

To prevent broken user authentication, use the setting value DEFAULT\_AUTHENTICATION\_CLASSES with the correct classes for your project and have authentication on every non-public API endpoint. Do not overwrite the authentication class on a class-based (variable
```
authentication_classes
```

) or function-based (decorator
```
authentication_classes
```
...
To prevent this problem, configure the setting DEFAULT\_THROTTLE\_CLASSES and DO NOT overwrite the throttle class on a class-based (variable
```
throttle_classes
```
...
To stop this problem, change the default value (
```
'rest_framework.permissions.AllowAny'
```

) of DEFAULT\_PERMISSION\_CLASSES. Use the setting value DEFAULT\_PERMISSION\_CLASSES with the correct classes for your project.
...
except for public API endpoints and DO NOT overwrite the authorization class on a class-based (variable
```
permission_classes
```
...
To prevent this problem, use Meta.fields (allowlist approach) when using ModelForms. DO NOT use Meta.exclude (denylist approach) or
```
...
**DO NOT use default passwords. Set the Django settings DEBUG and DEBUG\_PROPAGATE\_EXCEPTIONS to False. Ensure API can only be accessed by the specified HTTP verbs. All other HTTP verbs should be disabled. Set SECRET\_KEY to a random value and NEVER hardcode secrets.**
...
**To prevent this problem, use parametrized queries.** Be careful when using dangerous methods like
```
raw()
```
...
). DO NOT add user input to dangerous methods (
```
raw()
```
...
To stop this problem, use the
```
Loader=yaml.SafeLoader
```

for YAML files. DO NOT load user-controlled YAML files using the method
```
load()
```
...
Also, DO NOT add user input to dangerous methods (
```
eval()
```
...
) and DO NOT load user-controlled pickle files, which includes the pandas method
```
pandas.read_pickle()
```
...
## Updating Django and DRF and Having a Process for Updating Dependencies[¶](#updating-django-and-drf-and-having-a-process-for-updating-dependencies)
