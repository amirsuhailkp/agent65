...
- origin
- browser
date_collected: '2026-07-26T12:36:28.522394Z'
language: unknown
---

# HTML5 Security Cheat Sheet[¶](#html5-security-cheat-sheet)
...
- Don't assume you have control over the
  ...
  attribute. A single[Cross Site Scripting](Cross_Site_Scripting_Prevention_Cheat_Sheet.html)flaw in the sending page allows an attacker to send messages of any given format.
- Both pages should only interpret the exchanged messages as **data**. Never evaluate passed messages as code (e.g. via
  ```
  eval()
  ```
  ...
  ), as that would create a DOM-based XSS vulnerability. For more information see[DOM based XSS Prevention Cheat Sheet](DOM_based_XSS_Prevention_Cheat_Sheet.html).
...
- Keep in mind that CORS does not prevent the requested data from going to an unauthorized location. It's still important for the server to perform usual [CSRF](Cross-Site_Request_Forgery_Prevention_Cheat_Sheet.html)prevention.
- While the [Fetch Standard](https://fetch.spec.whatwg.org/#http-cors-protocol)recommends a pre-flight request with the
  ```
  OPTIONS
  ```
...
- Check out [WebSocket Security Cheat Sheet](WebSocket_Security_Cheat_Sheet.html)to learn about WebSocket specific protections.
...
- Web SQL Database was deprecated by the W3C in 2010 and is **removed from all major browsers**: Chromium dropped support in version 119 (October 2023) and Safari/Firefox never shipped it for third-party origins. Do not use Web SQL. If you specifically need an SQL interface in the browser, prefer running an embedded engine such as the official[SQLite WebAssembly build (](https://sqlite.org/wasm/doc/trunk/about.md), backed by IndexedDB or the Origin Private File System (OPFS) for persistence.
  ```
  sqlite-wasm
  ```

  )
- The current standard for client-side structured storage is , a transactional key-value store that has been a W3C Recommendation since 2015 and is supported in all evergreen browsers.[IndexedDB](https://developer.mozilla.org/en-US/docs/Web/API/IndexedDB_API)
...
- Validate messages exchanged with a Web Worker. Do not try to exchange snippets of JavaScript for evaluation e.g. via
  ...
  as that could introduce a[DOM Based XSS](DOM_based_XSS_Prevention_Cheat_Sheet.html)vulnerability.
...
to specify a [target loading location](https://www.w3schools.com/tags/att_a_target.asp) that does not replace the current location and then makes the current window/tab available.
...
- For the JavaScript
  ...
  in the[windowFeatures](https://developer.mozilla.org/en-US/docs/Web/API/Window/open)parameter of the
  ```
  window.open
  ```
...
- For [HTML links](https://www.scaler.com/topics/html/html-links/), add the attribute
  ```
  rel="noopener noreferrer"
  ```
...
- Add the HTTP response header
  ...
  to every HTTP response sent by the application ([Header Referrer-Policy information](https://owasp.org/www-project-secure-headers/). This configuration will ensure that no referrer information is sent along with requests from the page.
...
It is possible to have a [fine-grained control](https://html.spec.whatwg.org/multipage/iframe-embed-object.html#attr-iframe-sandbox) over
```
iframe
```
...
## Credential and Personally Identifiable Information (PII) Input hints[¶](#credential-and-personally-identifiable-information-pii-input-hints)
...
- The HTML5 Application Cache (
  ...
  files) has been**removed from all major browsers**(Firefox 85, Chrome 93). Do not use it for new applications and migrate any remaining usage to**Service Workers**with the[Cache API](https://developer.mozilla.org/en-US/docs/Web/API/Cache).
...
## Progressive Enhancements and Graceful Degradation Risks[¶](#progressive-enhancements-and-graceful-degradation-risks)
...
## HTTP Headers to enhance security[¶](#http-headers-to-enhance-security)
