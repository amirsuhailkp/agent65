...
- string
- new
date_collected: '2026-07-26T12:36:34.822504Z'
language: unknown
---

# Java Security Cheat Sheet[¶](#java-security-cheat-sheet)

## Injection Prevention in Java[¶](#injection-prevention-in-java)
...
### General advice to prevent Injection[¶](#general-advice-to-prevent-injection)

The following point can be applied, in a general way, to prevent *Injection* issue:

- Apply **Input Validation**(using allowlist approach) combined with**Output Sanitizing+Escaping**on user input/output.
...
Additional advice is provided on this [cheatsheet](Input_Validation_Cheat_Sheet.html).

## Specific Injection types[¶](#specific-injection-types)
...
### XML: XPath Injection[¶](#xml-xpath-injection)
...
### HTML/JavaScript/CSS[¶](#htmljavascriptcss)
...
outputToUser += "<img src='#' onload='javascript:alert(23);'>";
...
A dedicated [cheatsheet](LDAP_Injection_Prevention_Cheat_Sheet.html) has been created.
...
- Make sure [all XSS defenses](Cross_Site_Scripting_Prevention_Cheat_Sheet.html)are applied when viewing log files in a web browser.

#### Example using Log4j Core 2[¶](#example-using-log4j-core-2)
...
#### Example using Logback[¶](#example-using-logback)
...
### General cryptography guidance[¶](#general-cryptography-guidance)

- **Never, ever write your own cryptographic functions.**
- Wherever possible, try and avoid writing any cryptographic code at all. Instead try and either use pre-existing secret management solutions or the secret management solution provided by your cloud provider. For more information, see the [OWASP Secrets Management Cheat Sheet](Secrets_Management_Cheat_Sheet.html).
...
### Encryption for storage[¶](#encryption-for-storage)
...
#### Symmetric example using Google Tink[¶](#symmetric-example-using-google-tink)
...
#### Symmetric example using built-in JCA/JCE classes[¶](#symmetric-example-using-built-in-jcajce-classes)
...
### Encryption for transmission[¶](#encryption-for-transmission)
...
#### Asymmetric example using Google Tink[¶](#asymmetric-example-using-google-tink)
...
#### Asymmetric example using built-in JCA/JCE classes[¶](#asymmetric-example-using-built-in-jcajce-classes)
