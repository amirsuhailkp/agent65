...
- owasp-untrust
- classic
date_collected: '2026-07-26T12:44:57.642313Z'
language: unknown
---
...
Secure file system operations confined to a sandbox.
```
String
```
```
...
Strings with length and content restrictions.
```
int
```
...
Integer values bounded by a safe range.
```
File Name
```
...
Validates acceptable characters for file names.
```
String
```
...
Ensures safe characters for freeform text.
```
String
```
...
assumes full system access, whereas
```
BoxedPath
```

confines operations to a sandbox.
```
```
// Classic Path
...
Any attempt to escape the sandbox with
```
BoxedPath
```
...
Classic strings allow any content, which can introduce injection vulnerabilities.
```
FreeText
```
...
or excessive lengths are rejected at creation.

#### **3. Filename**

Classic filenames lack validation, leading to vulnerabilities with unsafe characters or malformed names.
```
Filename
```

enforces allowed characters and length constraints.
```
```
// Classic Filename
...
ensures integer values remain within a valid range.
```
```
// Classic Integer
...
- **Validation at Creation**: Unsafe data never enters the application logic.
- **Encapsulation of Constraints**: Security policies are enforced in reusable classes.
- **Minimal Code Changes**: Replacing classic constructs with secure equivalents requires minimal changes while drastically improving safety.
- **Self-Documenting Code**: Developers can understand constraints directly from type names (e.g.,
  ```
  FreeText
  ```
...
- **File System Operations**: Use
  ...
  for safe and confined file access.
- **User Input Handling**: Replace unvalidated strings with bounded types (
  ```
  UserName
  ```
...
- **Freeform Text**: Use
  ...
  to ensure safe and expected content.
