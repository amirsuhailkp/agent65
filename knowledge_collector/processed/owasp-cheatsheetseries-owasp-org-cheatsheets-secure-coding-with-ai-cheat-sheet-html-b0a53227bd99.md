...
- sheet
- secure
date_collected: '2026-07-26T12:36:54.904756Z'
language: unknown
---

# Secure Coding with AI Cheat Sheet[¶](#secure-coding-with-ai-cheat-sheet)
...
### Threat Actors and Attack Surfaces[¶](#threat-actors-and-attack-surfaces)
...
- **Rules files as persistent steering.**Files like
  ```
  .cursorrules
  ```
  ...
  silently steer every future generation. They can be modified by a malicious PR or by the agent itself to embed persistent instructions.
...
- **CI/CD agents.**Review bots and CI runners (e.g.
  ```
  claude-code-action
  ```

  , Copilot review) act on PR content with access to org secrets. A malicious PR can trigger the CI agent to exfiltrate secrets or modify the build pipeline. This is confused deputy at scale.

## Section 1: Hallucinated Dependencies[¶](#section-1-hallucinated-dependencies)
...
- Blindly run
  ...
  with package names suggested by an AI without verification.
...
## Section 2: Outdated Dependencies with Known CVEs[¶](#section-2-outdated-dependencies-with-known-cves)
...
## Section 3: Indirect Prompt Injection in the Development Loop[¶](#section-3-indirect-prompt-injection-in-the-development-loop)
...
## Section 4: MCP and Tool Security[¶](#section-4-mcp-and-tool-security)
...
## Section 5: Agent Runtime Sandboxing[¶](#section-5-agent-runtime-sandboxing)
...
- Understand and evaluate the risk of flags like
  ```
  --dangerously-skip-permissions
  ```

  or auto-accept modes. These bypass confirmation prompts and give the agent unrestricted execution.
...
## Section 6: Rules Files and Persistent Steering[¶](#section-6-rules-files-and-persistent-steering)
...
## Section 7: Out-of-Scope Edits and Review Anchoring[¶](#section-7-out-of-scope-edits-and-review-anchoring)
...
## Section 8: Test Fabrication and Test Deletion[¶](#section-8-test-fabrication-and-test-deletion)
...
## Section 9: Prompt Context Leakage and Sensitive Code Exposure[¶](#section-9-prompt-context-leakage-and-sensitive-code-exposure)
...
- Configure AI tools to exclude sensitive directories from context. Add
  ...
  *.key
  ...
  , and similar sensitive files to your AI tool's context exclusion list (
  ```
  .cursorignore
  ```
...
- Open
  ...
  files or private keys in your IDE while an AI coding assistant is active. The file contents may be sent as context.
...
- Assume that
  ...
  prevents AI tools from reading files.
  ```
  .gitignore
  ```
...
## Section 10: Prompt-to-Code Supply Chain Risk[¶](#section-10-prompt-to-code-supply-chain-risk)
...
- Review AI changes to the following files with heightened scrutiny -- treat them as security-critical:
  - ```
    ...
    (scripts section: postinstall, preinstall, prepare, prebuild)
  - ```
...
## Section 11: CI/CD Agents and Confused Deputy Risk[¶](#section-11-cicd-agents-and-confused-deputy-risk)
...
## Section 12: Markdown, Link, and Unicode Injection[¶](#section-12-markdown-link-and-unicode-injection)
...
## Section 13: Multi-Agent and Sub-Agent Propagation[¶](#section-13-multi-agent-and-sub-agent-propagation)
...
## Section 14: Human Accountability[¶](#section-14-human-accountability)
...
OWASP Top 10Relevant SectionA01: Broken Access ControlSection 5: Agent Runtime Sandboxing, Section 11: CI/CD AgentsA03: InjectionSection 3: Indirect Prompt Injection, Section 12: Markdown and Unicode InjectionA04: Insecure DesignSection 8: Test Fabrication, Section 14: Human AccountabilityA05: Security MisconfigurationSection 6: Rules Files, Section 7: Out-of-Scope EditsA06: Vulnerable and Outdated ComponentsSection 1: Hallucinated Dependencies, Section 2: Outdated DependenciesA07: Identification and Authentication FailuresSection 4: MCP and Tool SecurityA08: Software and Data Integrity FailuresSection 10: Prompt-to-Code Supply Chain RiskA09: Security Logging and Monitoring FailuresSection 9: Prompt Context Leakage, Section 11: CI/CD AgentsA10: Server-Side Request ForgerySection 3: Indirect Prompt Injection, Section 4: MCP and Tool Security
