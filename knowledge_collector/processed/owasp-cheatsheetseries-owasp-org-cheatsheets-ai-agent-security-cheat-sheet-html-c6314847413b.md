...
- tool
- message
date_collected: '2026-07-26T12:36:06.280419Z'
language: unknown
---

# AI Agent Security Cheat Sheet[¶](#ai-agent-security-cheat-sheet)
...
- **Prompt Injection (Direct & Indirect)**: Malicious instructions injected via user input or external data sources (websites, documents, emails) that hijack agent behavior. (See[LLM Prompt Injection Prevention Cheat Sheet](LLM_Prompt_Injection_Prevention_Cheat_Sheet.html))
...
### 1. Tool Security & Least Privilege[¶](#1-tool-security-least-privilege)
...
#### Bad: Over-permissioned Model Context Protocol (MCP) tool configuration[¶](#bad-over-permissioned-model-context-protocol-mcp-tool-configuration)
```
```
...
#### Good: Scoped MCP tool with allowlist[¶](#good-scoped-mcp-tool-with-allowlist)
```
```
...
#### Tool Authorization Middleware Example (Python)[¶](#tool-authorization-middleware-example-python)
```
```
...
### 2. Input Validation & Prompt Injection Defense[¶](#2-input-validation-prompt-injection-defense)
...
Please refer to the [LLM Prompt Injection Prevention Cheat Sheet](LLM_Prompt_Injection_Prevention_Cheat_Sheet.html) for detailed techniques.

### 3. Memory & Context Security[¶](#3-memory-context-security)
...
#### Bad: Unvalidated memory storage[¶](#bad-unvalidated-memory-storage)
```
```
...
#### Good: Validated and isolated memory[¶](#good-validated-and-isolated-memory)
```
```
import hashlib
...
#### Action Classification and Approval Flow[¶](#action-classification-and-approval-flow)
```
```
import uuid
...
#### High-Impact Action Integrity Controls[¶](#high-impact-action-integrity-controls)
...
### 5. Output Validation & Guardrails[¶](#5-output-validation-guardrails)
...
### 7. Multi-Agent Security[¶](#7-multi-agent-security)
...
#### Secure Multi-Agent Communication[¶](#secure-multi-agent-communication)
```
```
...
### 9. Secure Agent Testing & Adversarial Validation[¶](#9-secure-agent-testing-adversarial-validation)
...
#### Abuse-Case Test Matrix[¶](#abuse-case-test-matrix)

Maintain repeatable test cases for:

Abuse caseWhat to validatePrompt overrideSystem and developer instructions are not silently replaced by user or retrieved contentTool misuseUnauthorized tools are denied even when the model requests them confidentlyPrivilege escalationLow-trust sessions cannot reach privileged tools, credentials, or admin actionsMemory poisoningMalicious content is sanitized, scoped, expired, or rejected before persistenceData exfiltrationSensitive context is not leaked through tool calls, citations, logs, or final outputRecursive tool abuseChain depth, retry, token, and cost limits stop runaway loopsApproval bypassHigh-impact actions cannot execute without a valid, unexpired, parameter-bound approvalMulti-agent chainingOne compromised agent cannot cause another agent to exceed its trust boundary

#### CI/CD and Release Gates[¶](#cicd-and-release-gates)
