...
- sanctions
- agents
date_collected: '2026-07-26T12:36:08.039632Z'
language: unknown
---

# AML and Sanctions Compliance for AI Agent Payments Cheat Sheet[¶](#aml-and-sanctions-compliance-for-ai-agent-payments-cheat-sheet)
...
## Section 1: Agent Identity Before Screening[¶](#section-1-agent-identity-before-screening)

Before an agent is permitted to access sanctions screening services or initiate a payment, its identity must be cryptographically verified. Self-declared identity headers (e.g.
```
X-Agent-ID
```
...
) without cryptographic proof MUST be rejected. The message-level identity primitives in [Section 7 of the OWASP MCP Security Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/MCP_Security_Cheat_Sheet.html#7-message-level-integrity-and-replay-defence) apply directly to the agent-payment context.
...
## Section 2: Entity Screening[¶](#section-2-entity-screening)
...
## Section 3: Agent Operator Screening[¶](#section-3-agent-operator-screening)
...
## Section 4: Signed Audit Trail[¶](#section-4-signed-audit-trail)
...
- Rely on application-level logging (e.g.
  ```
  console.log
  ```

  , syslog) as the sole audit trail. These logs are not tamper-evident and can be modified without detection.
...
## Section 5: Fail-Closed Enforcement[¶](#section-5-fail-closed-enforcement)
...
## Section 6: Trust-Tiered Rate Limiting[¶](#section-6-trust-tiered-rate-limiting)
...
## Section 7: Self-Hosted vs Hosted Screening Architecture[¶](#section-7-self-hosted-vs-hosted-screening-architecture)
...
### Self-Hosted Screening[¶](#self-hosted-screening)
...
### Hosted Screening (Third-Party API)[¶](#hosted-screening-third-party-api)
...
## Section 8: Receipt Canonicalization (RFC 8785 / JCS)[¶](#section-8-receipt-canonicalization-rfc-8785-jcs)
...
Use the **JSON Canonicalization Scheme (JCS), RFC 8785**, to produce a deterministic byte representation before hashing and signing. The signer canonicalizes, hashes (for example with SHA-256), then signs; every verifier canonicalizes the received receipt identically and checks the signature. This makes receipts verifiable by

**any**counterparty, regulator, or downstream agent, not just the issuing system.
...
- Record the canonicalization, hash, and signature algorithm in the receipt (for example
  ```
  canon: jcs
  ```
  ...
  ) so any verifier can reproduce it.
...
## Section 9: Cross-Agent Payment Accountability[¶](#section-9-cross-agent-payment-accountability)
...
## Section 10: Binding Sanctions-List Freshness to the Receipt[¶](#section-10-binding-sanctions-list-freshness-to-the-receipt)
...
## Section 11: Regulatory Mapping[¶](#section-11-regulatory-mapping)
...
Control (this cheat sheet)Maps toAgent identity before screening (Section 1)KYC/KYB attribution; FATF Recommendation 10 (Customer Due Diligence)Entity and operator screening (Sections 2-3)OFAC, EU, UK, and UN sanctions screening; FATF Recommendation 6Signed audit trail and receipt (Sections 4, 8-10)Recordkeeping; FATF Recommendation 11; multi-year retention (BSA, EU AMLD)Sanctions-list freshness in receipt (Section 10)Obligation to screen against current lists; sanctions-evasion controlsFail-closed enforcement (Section 5)Blocking obligations for sanctioned partiesTrust-tiered limits (Section 6)Risk-based approach (FATF Recommendation 1); monitoring thresholds
...
## Section 12: Do's and Don'ts Summary[¶](#section-12-dos-and-donts-summary)
