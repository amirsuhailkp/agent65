...
- example
- will
date_collected: '2026-07-26T12:37:11.271981Z'
language: unknown
---

# Cross-site leaks Cheat Sheet[¶](#cross-site-leaks-cheat-sheet)
...
- ```
  ...
  - the cookie will be attached to a request from another site, but it must be sent over a secure HTTPS channel
- ```
  Lax
  ```

  - the cookie will be appended to the request from another page if the request method is GET and the request is made to top-level navigation (i.e. the navigation changes the address in the browser top bar)
- ```
  Strict  ```

  - the cookie will never be sent from another site
...
### How do we know that two sites are SameSite?[¶](#how-do-we-know-that-two-sites-are-samesite)
...
Why are we talking about eTLD+1 and not just TLD+1? It's because of domains like
...
. Such parts are not atomic enough to be compared well. For this reason, a list of "effective" TLDs (eTLDs) was created and can be found [here](https://publicsuffix.org/list/public_suffix_list.dat).

Sites that have the same eTLD+1 are considered SameSite, examples:

Origin AOrigin BSameSite?
...
## Attacks using the element ID attribute[¶](#attacks-using-the-element-id-attribute)
...
The browser will automatically focus on an element with a given ID if we append a hash to the URL, e.g.

```
https://example.com#pro```

. What's more, the JavaScript [focus event](https://developer.mozilla.org/en-US/docs/Web/API/Element/focus_event) gets fired. The attacker may try to embed the application in the iframe with specific source on its own controlled page:
...
#### Framing protection[¶](#framing-protection)
...
#### Fetch metadata (Sec-Fetch-Dest)[¶](#fetch-metadata-sec-fetch-dest)
...
## Attacks based on error events[¶](#attacks-based-on-error-events)
...
- ```
  ...
  - 200 OK - currently logged-in user is 1234 because we successfully loaded resource ([onload](https://developer.mozilla.org/en-US/docs/Web/API/GlobalEventHandlers/onload)event fired)
- ```
  GET /api/user/1235  ```

  - 401 Unauthorized - 1235 is not the ID of the currently logged in user ([onerror](https://developer.mozilla.org/en-US/docs/Web/API/GlobalEventHandlers/onerror)event will be triggered)
...
#### SubResource protection[¶](#subresource-protection)
...
#### Fetch metadata (Sec-Fetch-Site)[¶](#fetch-metadata-sec-fetch-site)
...
- ```
  ...
  - user directly reached the page
...
## Attacks on postMessage communication[¶](#attacks-on-postmessage-communication)
...
#### Specify strict targetOrigin[¶](#specify-strict-targetorigin)

To avoid situations like the one above, where an attacker manages to get the reference for a window to receive a message, always specify the exact
```
targetOrigin
```

in postMessage. Passing to the
```
targetOrigin
```
...
causes any origin to receive the message.
```
```
// Origin: http://example.com
const site = new URLSearchParams(window.location.search).get('site'); // https://evil.com
...
## Frame counting attacks[¶](#frame-counting-attacks)
...
An attacker can get information about the number of loaded frames in a window by counting the number of frames in a
```
window.frames
```
...
Setting this header will prevent cross-origin documents from opening in the same browsing context group. This solution ensures that document A opening another document will not have access to the
```
window
```
...
In case the server returns for example

```
same-origin```

COOP header, the attack fails:
...
## Attacks using browser cache[¶](#attacks-using-browser-cache)
...
#### Unpredictable tokens for images[¶](#unpredictable-tokens-for-images)
...
#### Using the Cache-Control header[¶](#using-the-cache-control-header)

You can disable the cache mechanism if you accept the degraded performance related to the necessity of reloading resources from the server every time a user visits the site. To disable caching for resources you want to protect, set the response header
