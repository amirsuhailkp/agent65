...
- input
- characters
date_collected: '2026-07-26T12:36:36.684083Z'
language: unknown
---

# LDAP Injection Prevention Cheat Sheet[¶](#ldap-injection-prevention-cheat-sheet)
...
### Defense Option 1: Escape all variables using the right LDAP encoding function[¶](#defense-option-1-escape-all-variables-using-the-right-ldap-encoding-function)

#### Distinguished Name Escaping[¶](#distinguished-name-escaping)
...
The [exhaustive list](https://ldapwiki.com/wiki/Wiki.jsp?page=DN%20Escape%20Values) is the following:
```
\ # + < > , ; " =
```
...
#### Search Filter Escaping[¶](#search-filter-escaping)
...
#### Safe Java Escaping Example[¶](#safe-java-escaping-example)
...
#### Insecure vs Secure Java LDAP Query Construction[¶](#insecure-vs-secure-java-ldap-query-construction)
...
[.NET AntiXSS](https://blogs.msdn.microsoft.com/securitytools/2010/09/30/antixss-4-0-released/) (now the Encoder class) has LDAP encoding functions including
```
...
encodes input according to [RFC4515](https://datatracker.ietf.org/doc/html/rfc4515) where unsafe values are converted to
```
\XX
```
...
encodes input according to [RFC2253](https://tools.ietf.org/html/rfc2253) where unsafe characters are converted to
```
#XX
```
...
is the representation of the unsafe character and the comma, plus, quote, slash, less than and great than signs are escaped using slash notation (
```
\X
```

). In addition to this a space or octothorpe (
```
#
```

) at the beginning of the input string is
```
\
```

escaped as is a space at the end of a string.
```
LdapDistinguishedNameEncode(string, bool, bool)
```
...
### Defense Option 2: Use Frameworks that Automatically Protect from LDAP Injection[¶](#defense-option-2-use-frameworks-that-automatically-protect-from-ldap-injection)
...
### Enabling Bind Authentication[¶](#enabling-bind-authentication)
...
Input validation can be used to detect unauthorized input before it is passed to the LDAP query. For more information please see the [Input Validation Cheat Sheet](Input_Validation_Cheat_Sheet.html).
