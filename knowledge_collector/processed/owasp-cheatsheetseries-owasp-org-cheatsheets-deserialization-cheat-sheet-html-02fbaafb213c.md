...
- used
- java
date_collected: '2026-07-26T12:36:22.404300Z'
language: unknown
---
...
## Guidance on Deserializing Objects Safely[¶](#guidance-on-deserializing-objects-safely)
...
#### Clear-box Review[¶](#clear-box-review)

Check the use of  [unserialize()](https://www.php.net/manual/en/function.unserialize.php) function and review how the external parameters are accepted. Use a safe, standard data interchange format such as JSON (via
```
json_decode()
```
...
#### Opaque-box Review[¶](#opaque-box-review)
...
- In your code, override the
  ...
  method to prevent arbitrary classes from being deserialized. This safe behavior can be wrapped in a library like[SerialKiller](https://github.com/ikkisoft/SerialKiller).
- Use a safe replacement for the generic
  ```
  readObject()
  ```

  method as seen here. Note that this addresses "[billion laughs](https://en.wikipedia.org/wiki/Billion_laughs_attack)" type attacks by checking input length and number of objects deserialized.
...
#### Prevent Data Leakage and Trusted Field Clobbering[¶](#prevent-data-leakage-and-trusted-field-clobbering)

If there are data members of an object that should never be controlled by end users during deserialization or exposed to users during serialization, they should be declared as [the transient keyword](https://docs.oracle.com/javase/7/docs/platform/serialization/spec/serial-arch.html#7231) (section
...
#### Prevent Deserialization of Domain Objects[¶](#prevent-deserialization-of-domain-objects)
...
class is guaranteed to **not** deserialize any other type besides the
...
#### Other Deserialization Libraries and Formats[¶](#other-deserialization-libraries-and-formats)
...
- (JSON) - can be used safely as long as the[fastjson2](https://github.com/alibaba/fastjson2)option is not turned on**autotype**
- (JSON) - can be used safely as long as polymorphism is not used ([jackson-databind](https://github.com/FasterXML/jackson-databind)[see blog post](https://cowtowncoder.medium.com/on-jackson-cves-dont-panic-here-is-what-you-need-to-know-54cd0d6e8062))
- (custom format) - can be used safely as long as class registration is not turned[Kryo v5.0.0+](https://github.com/EsotericSoftware/kryo)**off**([see documentation](https://github.com/EsotericSoftware/kryo#optional-registration)and[this issue](https://github.com/EsotericSoftware/kryo/issues/929))
- (YAML) - can be used safely as long as the[YamlBeans v1.16+](https://github.com/EsotericSoftware/yamlbeans)**UnsafeYamlConfig**class isn't used (see[this commit](https://github.com/EsotericSoftware/yamlbeans/commit/b1122588e7610ae4e0d516c50d08c94ee87946e6))
...
- (JSON and XML) - can be used safely as long as the allowlist and other security controls are not relaxed ([XStream v1.4.17+](https://x-stream.github.io/)[see documentation](https://x-stream.github.io/security.html))
...
- (JSON) - cannot be used safely unless the[fastjson v1.2.68+](https://github.com/alibaba/fastjson)option is turned on, which disables deserialization of any class (**safemode**[see documentation](https://github.com/alibaba/fastjson/wiki/enable_autotype)). Previous versions are not safe.
- (JSON) - cannot be used safely since the use of[json-io](https://github.com/jdereg/json-io)**@type**property in JSON allows deserialization of any class. Can only be used safely in following situations:
  - In [non-typed mode](https://github.com/jdereg/json-io/blob/master/user-guide.md#non-typed-usage)using the**JsonReader.USE\_MAPS**setting which turns off generic object deserialization
...
- (YAML) - cannot be used safely unless the[SnakeYAML](https://bitbucket.org/snakeyaml/snakeyaml/src)**org.yaml.snakeyaml.constructor.SafeConstructor**class is used, which disables deserialization of any class ([see docs](https://bitbucket.org/snakeyaml/snakeyaml/wiki/CVE-2022-1471))

**Cannot be used safely:**

The following libraries are either no longer maintained or cannot be used safely with untrusted input:

- (XML) - appears to be abandoned with no commits since 2016[Castor](https://github.com/castor-data-binding/castor)
- (JSON) - these versions allows deserialization of any class ([fastjson < v1.2.68](https://github.com/alibaba/fastjson)[see documentation](https://github.com/alibaba/fastjson/wiki/enable_autotype))
- (XML) -[XMLDecoder in the JDK](https://docs.oracle.com/javase/8/docs/api/java/beans/XMLDecoder.html)*"close to impossible to securely deserialize Java objects in this format from untrusted inputs"*("Red Hat Defensive Coding Guide",[end of section 2.6.5](https://redhat-crypto.gitlab.io/defensive-coding-guide/#sect-Defensive_Coding-Tasks-Serialization-XML))
- (JSON and XML) - these versions allows deserialization of any class (see[XStream < v1.4.17](https://x-stream.github.io/)[documentation](https://x-stream.github.io/security.html#explicit))
- (YAML) - these versions allows deserialization of any class (see[YamlBeans < v1.16](https://github.com/EsotericSoftware/yamlbeans)[this document](https://github.com/Contrast-Security-OSS/yamlbeans/blob/main/SECURITY.md))
...
type is dangerous and cannot be secured. As such, it should not be used. Full details are in the [BinaryFormatter security guide](https://docs.microsoft.com/en-us/dotnet/standard/serialization/binaryformatter-security-guide).
...
## Language-Agnostic Methods for Deserializing Safely[¶](#language-agnostic-methods-for-deserializing-safely)
...
## Mitigation Tools/Libraries[¶](#mitigation-toolslibraries)
