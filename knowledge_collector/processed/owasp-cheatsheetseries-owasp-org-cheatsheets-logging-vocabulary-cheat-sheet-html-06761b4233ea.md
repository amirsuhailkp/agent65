...
- user
- joebob1
date_collected: '2026-07-26T12:36:39.394807Z'
language: unknown
---

# Application Logging Vocabulary Cheat Sheet[¶](#application-logging-vocabulary-cheat-sheet)
...
- Python / Java / Go / Node.js: [thatsjet/security\_event\_logger](https://github.com/thatsjet/security_event_logger)
...
### authn\_login\_success[:userid][¶](#authn_login_successuserid)
...
### authn\_login\_successafterfail[:userid,retries][¶](#authn_login_successafterfailuseridretries)
...
### authn\_login\_fail[:userid][¶](#authn_login_failuserid)
...
### authn\_login\_fail\_max[:userid,maxlimit(int)][¶](#authn_login_fail_maxuseridmaxlimitint)
...
### authn\_login\_lock[:userid,reason][¶](#authn_login_lockuseridreason)
...
### authn\_password\_change[:userid][¶](#authn_password_changeuserid)
...
### authn\_password\_change\_fail[:userid][¶](#authn_password_change_failuserid)

**Description**
An attempt to change a password that failed. May also trigger other events such as
```
authn_login_lock
```
...
### authn\_impossible\_travel[:userid,region1,region2][¶](#authn_impossible_traveluseridregion1region2)
...
### authn\_token\_created[:userid, entitlement(s)][¶](#authn_token_createduserid-entitlements)
...
### authn\_token\_revoked[:userid,tokenid][¶](#authn_token_revokeduseridtokenid)
...
### authn\_token\_reuse[:userid,tokenid][¶](#authn_token_reuseuseridtokenid)
...
### authn\_token\_delete[:appid][¶](#authn_token_deleteappid)
...
### authz\_fail[:userid,resource][¶](#authz_failuseridresource)
...
### authz\_change[:userid,from,to][¶](#authz_changeuseridfromto)
...
### authz\_admin[:userid,event][¶](#authz_adminuseridevent)
...
## Encryption/Decryption [CRYPT][¶](#encryptiondecryption-crypt)

### crypt\_decrypt\_fail[userid][¶](#crypt_decrypt_failuserid)
...
### crypt\_encrypt\_fail[userid][¶](#crypt_encrypt_failuserid)
...
## Excessive Use [EXCESS][¶](#excessive-use-excess)

### excess\_rate\_limit\_exceeded[userid,max][¶](#excess_rate_limit_exceededuseridmax)
...
### excess\_sessions\_exceeded[userid,max][¶](#excess_sessions_exceededuseridmax)
...
### upload\_complete[userid,filename,type][¶](#upload_completeuseridfilenametype)
...
### upload\_stored[filename,from,to][¶](#upload_storedfilenamefromto)
...
### upload\_validation[filename,(virusscan|imagemagick|...):(FAILED|incomplete|passed)][¶](#upload_validationfilenamevirusscanimagemagickfailedincompletepassed)
...
### upload\_delete[userid,fileid][¶](#upload_deleteuseridfileid)
...
## Input Validation [INPUT][¶](#input-validation-input)

### input\_validation\_fail:[(fieldone,fieldtwo...),userid][¶](#input_validation_failfieldonefieldtwouserid)
...
### input\_validation\_discrete\_fail[:field,userid][¶](#input_validation_discrete_failfielduserid)
...
## Malicious Behavior [MALICIOUS][¶](#malicious-behavior-malicious)

### malicious\_excess\_404:[userid|IP,useragent][¶](#malicious_excess_404useridipuseragent)
...
### malicious\_extraneous:[userid|IP,inputname,useragent][¶](#malicious_extraneoususeridipinputnameuseragent)
...
### malicious\_attack\_tool:[userid|IP,toolname,useragent][¶](#malicious_attack_tooluseridiptoolnameuseragent)
...
### malicious\_sqli:[userid|IP,parameter,ruleid,useragent][¶](#malicious_sqliuseridipparameterruleiduseragent)

**Description**
When request input matches a SQL injection (SQLi) signature or heuristic (e.g., comment delimiters, tautologies like
```
' OR 1=1 --
```
...
, etc.), block the request and log the attempt.

*NOTE: Logging the payload is dangerous and may result in log injection. Prefer recording a detection rule ID / category and the parameter name over logging the full payload.*
...
### malicious\_cors:[userid|IP,useragent,referer][¶](#malicious_corsuseridipuseragentreferer)
...
### malicious\_direct\_reference:[userid|IP, useragent][¶](#malicious_direct_referenceuseridip-useragent)
...
### malicious\_csrf:[userid|IP][¶](#malicious_csrfuseridip)
...
### malicious\_csp\_violation:[userid|IP,effective\_directive,blocked\_uri,useragent][¶](#malicious_csp_violationuseridipeffective_directiveblocked_uriuseragent)
...
*NOTE: CSP report fields such as document-uri, referrer, and blocked-uri are URL-valued (see the*

[W3C Content Security Policy Level 3](https://www.w3.org/TR/CSP3/#violation-events)violation report definition) and routinely contain full URLs whose query strings carry session or reset tokens. Strip the query string/fragment before logging, and prefer the effective-directive + blocked-uri (origin+path) over the full report body to avoid a secondary sensitive-data or log-injection risk.
...
### mcp\_prompt\_injection[:userid][¶](#mcp_prompt_injectionuserid)
...
*NOTE: Avoid logging the full prompt/tool I/O. Prefer logging a detection category or rule ID, the target tool/server, and request identifiers to support triage without creating a secondary sensitive-data or log-injection risk.*
...
### mcp\_resource\_exhaustion[:userid][¶](#mcp_resource_exhaustionuserid)
...
*NOTE: Avoid logging full prompts or tool inputs/outputs. Prefer logging measured usage (tokens in/out), policy thresholds, request identifiers, and the tool/model involved.*
...
### mcp\_tool\_poisoning[:userid][¶](#mcp_tool_poisoninguserid)
...
*NOTE: Tool ecosystems and agent marketplaces can enable rapid proliferation of third-party tools with relatively low validation. An internal marketplace or hashed allowlist for approved tools is highly recommended.*
...
## Privilege Changes [PRIVILEGE][¶](#privilege-changes-privilege)
...
### privilege\_permissions\_changed:[userid,file|object,fromlevel,tolevel][¶](#privilege_permissions_changeduseridfileobjectfromleveltolevel)
...
## Sensitive Data Changes [DATA][¶](#sensitive-data-changes-data)
...
### sensitive\_create:[userid,file|object][¶](#sensitive_createuseridfileobject)
...
### sensitive\_read:[userid,file|object][¶](#sensitive_readuseridfileobject)
...
### sensitive\_update:[userid,file|object][¶](#sensitive_updateuseridfileobject)
...
### sensitive\_delete:[userid,file|object][¶](#sensitive_deleteuseridfileobject)
...
## Sequence Errors [SEQUENCE][¶](#sequence-errors-sequence)
...
### sequence\_fail:[userid][¶](#sequence_failuserid)
...
### session\_created:[userid][¶](#session_createduserid)
...
### session\_renewed:[userid][¶](#session_reneweduserid)
...
### session\_expired:[userid,reason][¶](#session_expireduseridreason)
...
### session\_logout:[userid,sessionid][¶](#session_logoutuseridsessionid)

**Description**
When a user explicitly logs out (as opposed to a timeout or administrative revocation) the event may be logged. This is a more explicit alternative to
```
session_expired:[userid,logout]
```

for systems that distinguish a user-initiated logout.
...
### session\_use\_after\_expire:[userid][¶](#session_use_after_expireuserid)
...
## System Events [SYS][¶](#system-events-sys)

### sys\_startup:[userid][¶](#sys_startupuserid)
...
### sys\_shutdown:[userid][¶](#sys_shutdownuserid)
...
### sys\_restart:[userid][¶](#sys_restartuserid)
...
### sys\_crash[:reason][¶](#sys_crashreason)
...
### sys\_monitor\_disabled:[userid,monitor][¶](#sys_monitor_disableduseridmonitor)
...
### sys\_monitor\_enabled:[userid,monitor][¶](#sys_monitor_enableduseridmonitor)
...
### user\_created:[userid,newuserid,attributes[one,two,three]][¶](#user_createduseridnewuseridattributesonetwothree)
...
### user\_updated:[userid,onuserid,attributes[one,two,three]][¶](#user_updateduseridonuseridattributesonetwothree)
...
### user\_archived:[userid,onuserid][¶](#user_archiveduseridonuserid)
...
### user\_deleted:[userid,onuserid][¶](#user_deleteduseridonuserid)
